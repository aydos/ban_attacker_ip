...
sort iplist | uniq -c | awk '{if($1==1) print $2}' | xargs -I IP firewall-cmd --zone=public --remove-rich-rule='rule family="ipv4" source address="IP" reject'

# her ip'den bir tanesini siler
sort iplist -u | xargs -I ip sed -i '0,/'ip'/{//d;}' iplist
...
sort newips -u | xargs -I IP firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" source address="IP" reject'
